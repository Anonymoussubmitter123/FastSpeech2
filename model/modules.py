...
class VarianceAdaptor(nn.Module):
    ...
    def __init__(self, preprocess_config, model_config):
        ...
        if pitch_quantization == "log":
            ...
            self.pitch_bins = nn.Parameter(
                ...
                # start参数描述：序列的起始值，stop参数描述：序列的结束值，
                # num参数描述：要生成的步长，通俗来讲就是该序列的数量。
                requires_grad=False,
            )
    ...
    def forward(
        ...
        if self.pitch_feature_level == "frame_level":  # 帧级别
            pitch_prediction, pitch_embedding = self.get_pitch_embedding(
...
class LengthRegulator(nn.Module):
    ...
    def LR(self, x, duration, max_len):
        ...
        if max_len is not None:
            ...
            output = pad(output)  # 常⽤于深度学习中的数据预处理，可以将numpy数组按指定的⽅法填充成指定的形状。
    ...
    def expand(self, batch, predicted):
        ...
        for i, vec in enumerate(batch):
            ...
            """
            expand_size = predicted[i].item()  # item()的作用是取出单元素张量的元素值并返回该值，保持该元素类型不变。
